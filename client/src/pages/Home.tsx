...
function ReadingView({ story, storyWords, processedWords, state, recognitionStatus, transcript, isProcessing, assessmentMode, wordStates, hesitationHint, onMoveOn, onBack, onStart, onPauseResume, onComplete }: { story: Story; storyWords: string[]; processedWords: number; state: ReadingState; recognitionStatus: RecognitionStatus; connectionStatus: ConnectionStatus; transcript: string; isProcessing: boolean; coachMoment: CoachMoment; assessmentMode: AssessmentMode; wordStates: WordState[]; hesitationHint: boolean; onModeChange: (mode: AssessmentMode) => void; onWordAttempt: (correct: boolean) => void; onMoveOn: () => void; onBack: () => void; onStart: () => void; onPauseResume: () => void; onRestart: () => void; onComplete: () => void; onGuidedComplete: () => void; onCoach: (moment: CoachMoment) => void }) {
  ...
  const hearModel = (text: string) => {
    if (state === "listening") onPauseResume();
    setIsModelSpeaking(true);
    if (!playSpeech(text, undefined, () => setIsModelSpeaking(false))) setIsModelSpeaking(false);
  };
  return <div className="kids-reading-shell"><header className="kids-reading-head"><button type="button" className="back-link" onClick={onBack}><ArrowLeft size={17} /> Back to Reading Library</button><div><h1 className="story-label">{story.title}</h1><p>Page 1 of 3</p></div><div className="kids-progress" aria-label={`${progress}% through the story`}><i style={{ width: `${progress}%` }} /></div></header><section className="kids-reader-stage"><div className="reader-text kids-reader-text" aria-label="Reading passage">{storyWords.map((word, index) => { const wordState = wordStates[index]; const stateClass = assessmentMode === "MONTHLY_ASSESSMENT" ? (index === processedWords && isListening ? "current" : "") : wordState?.status ?? (index < processedWords ? "correct" : index === processedWords && isListening ? "current" : "unread"); const isHintWord = showWordHint && wordState?.id === currentWord?.id; return <span className={`reader-word ${stateClass} ${isHintWord ? "needs-help" : ""}`} key={`${word}-${index}`}>{word}{wordState?.status === "retried_correct" && assessmentMode !== "MONTHLY_ASSESSMENT" ? <i className="retry-check" aria-label="Self-corrected"><Check size={11} /></i> : null}{isHintWord ? <button type="button" className="word-help-tooltip" onClick={() => hearModel(word.replace(/[^a-zA-Z']/g, ""))}>Tap word to hear sound <Volume2 size={13} /></button> : null}</span>; })}</div>{hesitationHint && assessmentMode !== "MONTHLY_ASSESSMENT" && <div className="kids-gentle-pause">A small pause is okay. Take a breath and keep going.</div>}</section><div className="kids-control-bar"><button type="button" className="hear-page-button" onClick={() => hearModel(story.text)} aria-pressed={isModelSpeaking}><Volume2 size={19} /><span>{isModelSpeaking ? "Reading aloud…" : "Hear page"}</span></button>{state === "ready" ? <button type="button" className="kids-mic-button" onClick={onStart}><Mic size={31} /><span>Tap to Read</span></button> : <button type="button" className={`kids-mic-button ${isListening ? "listening" : ""}`} onClick={onPauseResume}>{isListening ? <><Pause size={29} fill="currentColor" /><span>Pause</span></> : <><Play size={29} fill="currentColor" /><span>Keep reading</span></>}</button>}<button type="button" className="finish-button kids-finish-button" onClick={onComplete} disabled={isProcessing}>{isProcessing ? "Getting report…" : "Finish story"}</button></div>{showMoveOn && <div className="kids-move-on-card" role="status"><b>That word can wait for later.</b><span>We will save it for a short word activity after your story.</span><button type="button" onClick={onMoveOn}>Need help—move on <ChevronRight size={16} /></button></div>}<span className="sr-only" role="status" aria-live="polite">{recognitionStatus === "unavailable" ? "Guided reading is ready." : isListening ? "Listening." : state === "processing" ? "Preparing your reading report." : "Ready to read."}</span></div>;
}
